...
//				// 以商品ID+订单ID+商品价格作为相同商品的依据
//				Map<String, List<UserOrder>> userOrderMap = new HashMap<>();
//				for (UserOrder userOrder2 : userOrderList) {
//					// 判断相同orderId+price
//					if (userOrderMap
//							.get(userOrder2.getProductId()+"-" + userOrder2.getOrderId()+"-" + userOrder2.getPrice()) == null) {
//						List<UserOrder> uOrder = new ArrayList<>();
//						uOrder.add(userOrder2);
//						userOrderMap.put(userOrder2.getProductId()+"-" + userOrder2.getOrderId()+"-" + userOrder2.getPrice(),
//								uOrder);
//					} else {
//						userOrderMap.get(userOrder2.getOrderId()).add(userOrder2);
//					}
//				}
//				for (String key : userOrderMap.keySet()) {
//					String [] keys = key.split("-");
//					Map<String, Object> map = new HashMap<>();
//					map.put("productId", keys[0]);
//					map.put("orderId", keys[1]);
//					map.put("price", keys[2]);
...
//					map.put("price", userOrder1.getPrice());
					//从淘宝导入的订单有可能会出现同一个订单号下面有多条相同商品记录
					List<TkOrderInput> tkOrderInputList = tkOrderInputService.selectByMap(map);
					double payMoney = 0;
					...
					if (tkOrderInputList != null && tkOrderInputList.size()>0) {
						TkOrderInput tkOrderInputMatch=null;
						for(TkOrderInput tkOrderInput:tkOrderInputList){							
							if ((userOrder1.getProductId()).equals(tkOrderInput.getProductId())
									&& (userOrder1.getOrderId()).equals(tkOrderInput.getOrderId())
									&& (String.valueOf(userOrder1.getPrice())).equals(String.valueOf(tkOrderInput.getPrice()))
									&& userOrder1.getProductNum()==tkOrderInput.getProductNum()
									&& (userOrder1.getProductInfo()).equals(tkOrderInput.getProductInfo())) {
								tkOrderInputMatch=tkOrderInput;
								break;
							}
						}
						if(tkOrderInputMatch!=null){
							TkOrderInput tkOrderInput=tkOrderInputMatch;

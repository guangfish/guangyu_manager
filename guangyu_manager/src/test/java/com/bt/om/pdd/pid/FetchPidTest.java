...
import com.bt.om.pdd.pid.vo.JsonRootBean;
import com.bt.om.pdd.pid.vo.P_id_list;
...
public class FetchPidTest extends AbstractJUnit4SpringContextTests {
	...
	public void createPid() {
		...
		while (true) {
			...
			String dd = "";
			String sign = "";
			dd = client_secret + "client_id" + client_id + "data_type" + data_type + "page" + page + "page_size" + size
					+ "timestamp" + timestamp + "type" + type + client_secret;
			sign = getMD5String(dd);
			sign = sign.toUpperCase();
			remoteTaskUrl = remoteTaskUrl + "?type=" + type + "&sign=" + sign + "&client_id=" + client_id
					+ "&data_type=" + data_type + "&timestamp=" + timestamp + "&page=" + page + "&page_size=" + size;
			try {
				String ret = HttpcomponentsUtil.sendHttpsJson(nvpList, remoteTaskUrl);
//				System.out.println(ret);
				JsonRootBean root = GsonUtil.GsonToBean(ret, JsonRootBean.class);
				List<P_id_list> list = root.getP_id_query_response().getP_id_list();
